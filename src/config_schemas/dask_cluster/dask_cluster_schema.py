...
class GCPDaskClusterConfig(DaskClusterConfig):
    ...
    extra_bootstrap: Optional[list[str]] = field(
        default_factory=lambda: ["gcloud auth configure-docker --quiet asia-southeast1-docker.pkg.dev"]
    )
...
def setup_config() -> None:
    ...
    cs.store(name="local_dask_cluster_schema", group="dask_cluster", node=LocalDaskClusterConfig)
    cs.store(name="gcp_dask_cluster_schema", group="dask_cluster", node=GCPDaskClusterConfig)